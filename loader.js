...
(function(){
  ...
  { addr: "Contact us on this Discord: https://discord.gg/UEE39zHuCx", name: "Want to see your server here?" }
        ...
        // Uses the SAME delegation pattern as the original loader.js.
        // Only intercepts onclose to detect failure and trigger proxy fallback.
        // Everything else delegates directly to the native WebSocket.
        ...
        window.WebSocket = class {
                constructor(uri, protos) {
                        ...
                        var self = this;
                        try {
                                this.impl = (protos != null) ? new nativeWS(uri, protos) : new nativeWS(uri);
                        } catch(e) {
                                this.impl = null;
                                this._tryProxy();
                                return;
                        ...
                        if(this.vigg) {
                                setTimeout(() => { try { this.impl.close(); } catch(_) {} }, 100000 + Math.floor(20000 * Math.random()));
                        }
                        // Set up intercepting handlers on the native WS
                        ...
                        this.impl.onclose = function(e) {
                                if (self._directOpened || self._triedProxy) {
                ...
                _tryProxy() {
                        ...
                        if (self.impl) { try { self.impl.close(); } catch(_) {} self.impl = null; }
                        if (!PROXY_URL) {
                                if (self._userOnError) { try { self._userOnError.call(self, { type: 'error' }); } catch(_) {} }
                                if (self._userOnClose) { try { self._userOnClose.call(self, { type: 'close', code: 1006, reason: 'Connection failed', wasClean: false }); } catch(_) {} }
                                return;
                        }
                        ...
                        try {
                                self.impl = (self._origProtos != null) ? new nativeWS(proxyUrl, self._origProtos) : new nativeWS(proxyUrl);
                        } catch(e) {
                                if (self._userOnError) { try { self._userOnError.call(self, { type: 'error' }); } catch(_) {} }
                                if (self._userOnClose) { try { self._userOnClose.call(self, { type: 'close', code: 1006, reason: 'Proxy failed', wasClean: false }); } catch(_) {} }
                                return;
                        }
                        self.impl.onopen = function(e) { if (self._userOnOpen) { try { self._userOnOpen.call(self, e); } catch(_) {} } };
                        self.impl.onmessage = function(e) { if (self._userOnMessage) { try { self._userOnMessage.call(self, e); } catch(_) {} } };
                        self.impl.onclose = function(e) { if (self._userOnClose) { try { self._userOnClose.call(self, e); } catch(_) {} } };
                        self.impl.onerror = function(e) { if (self._userOnError) { try { self._userOnError.call(self, e); } catch(_) {} } };
                }
                // Delegate everything to impl (same as original loader.js)
                get binaryType() { return this.impl ? this.impl.binaryType : 'blob'; }
                set binaryType(v) { if (this.impl) { this.impl.binaryType = v; } }
                get readyState() { return this.impl ? this.impl.readyState : 0; }
                get url() { return this.impl ? this.impl.url : this._origUrl; }
                get protocol() { return this.impl ? this.impl.protocol : ''; }
                get extensions() { return this.impl ? this.impl.extensions : ''; }
                get bufferedAmount() { return this.impl ? this.impl.bufferedAmount : 0; }
                // User callbacks are stored, not delegated (because we intercept them)
                set onopen(v) { this._userOnOpen = v; }
                set onclose(v) { this._userOnClose = v; }
                set onmessage(v) { this._userOnMessage = v; }
                set onerror(v) { this._userOnError = v; }
                close(c, r) { if (this.impl) { try { this.impl.close(c, r); } catch(_) {} } }
                send(p) {
                        if(this.vigg) { if(Math.random() < ((Date.now() - this.startTime) / 120000 - 0.25) * 0.5) return; }
                        if (this.impl) { try { this.impl.send(p); } catch(_) {} }
                }
                addEventListener(p1, p2) { if (this.impl) { this.impl.addEventListener(p1, p2); } }
                removeEventListener(p1, p2) { if (this.impl) { this.impl.removeEventListener(p1, p2); } }
        };
